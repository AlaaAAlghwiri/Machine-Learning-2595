---
title: "Intro to Machine Learning Spring 2025 Homework: 4"
subtitle: "Assigned February 17, 2025; Due: February 28, 2025"
...
Answer:
Based on the inputs of the question, we can formulate the following equations:

$$
115 = \mu_{0} - 2 \times \tau_{0}
$$

$$
135 = \mu_{0} + 2 \times \tau_{0}
$$
Add the previous two formulas to each other, we then get:
$$
250 = 2 \times \mu_{0}
$$
Solve for $\mu_{0}$
...
\mu_{0} = 125
$$
Substitute $\mu_{0} = 125$ in the first equation:
$$
\tau_{0} = 5
$$
...
Answer:
Here is the 25th quantiles
```{r}
qnorm(p = 0.25, mean = 125, sd = 5)
```
And here is the 75th quantiles
```{r}
...
Answer:
```{r}
paste0("The prior probability that the mean performance, mu is less than 117 = ", round(pnorm(117, mean = 125, sd = 5), 3))
```
...
Answer:
The posterior precision is the sum of the prior and the data precision:

$$
\frac{1}{\tau_{N}^{2}} = \frac{1}{\tau_{0}^{2}} + \frac{N}{\sigma^{2}}
$$
```{r}
get_nobser <- function(tau_0, sigma, tau_N){
  N <- (1 / tau_N^2 - 1 / tau_0^2) * sigma^2
  return(N)
}
...
After plugging in all numbers to the function, we get $N = 5.25$. So we need 6 observations to achieve $\tau_{N} = 2$.

Let's try another 2 $\tau_{N}$ values:[1, 0.5]
```{r}
get_nobser(5, 5, 1)
```
So we need 24 observations to achieve $\tau_{N} = 1$
```{r}
get_nobser(5, 5, 0.5)
```
So we need 99 observations to achieve $\tau_{N} = 0.5$
...
You may add as many equation blocks and code chunks as you see fit to answer this question.
Answer:
In this case, following the same formula as in the previous question, we get:
```{r}
get_nobser(Inf, 15, 3)
```
We will need *25* parts in this case.
...
$$
\log\Big( \mathcal{N}(x_n \mid \mu, \sigma) \Big)
...
$$
\log\Big( \mathcal{N}(x_n \mid \mu, \sigma) \Big)
\propto -\frac{1}{2\sigma^2}(x_n - \mu)^2.
$$


$$
\log\Big( p(\mathbf{x} \mid \mu, \sigma) \Big)
\propto -\frac{1}{2\sigma^2} \sum_{n=1}^{N} (x_n - \mu)^2.
$$


$$
\log\Big( \mathcal{N}(\mu \mid \mu_0, \tau_0) \Big)
...
$$
\log\Big( p(\mu \mid \mathbf{x}, \sigma) \Big)
\propto -\frac{1}{2\sigma^2} \sum_{n=1}^{N} (x_n - \mu)^2
...
$$
\frac{d}{d\mu}\Big(\log\big(p(\mu \mid \mathbf{x}, \sigma)\big)\Big)
= -\frac{1}{2\sigma^2} \sum_{n=1}^{N} \frac{d}{d\mu}\Big((x_n-\mu)^2\Big)
- \frac{1}{2\tau_0^2}\frac{d}{d\mu}\Big((\mu-\mu_0)^2\Big)
$$

Let’s start with the term associated with the likelihood:
$$
\frac{d}{d\mu}\Big((x_n-\mu)^2\Big) = -2\,(x_n-\mu)
$$

And similarily for the log prior term:
$$
\frac{d}{d\mu}\Big((\mu-\mu_0)^2\Big) = 2\,(\mu-\mu_0)
$$

$$
\frac{d}{d\mu}\Big(\log\big(p(\mu \mid \mathbf{x}, \sigma)\big)\Big)
= -\frac{1}{2\sigma^2} \sum_{n=1}^{N} \Big(-2\,(x_n-\mu)\Big)
- \frac{1}{2\tau_0^2}\cdot 2\,(\mu-\mu_0)
$$
...
$$
\frac{d}{d\mu}\Big(\log\big(p(\mu \mid \mathbf{x}, \sigma)\big)\Big)
= \frac{1}{\sigma^2}\Big(N\,\bar{x} - N\,\mu\Big)
...
Answer:
$$
\frac{d}{d\mu}\Big(\log\big(p(\mu\mid \mathbf{x}, \sigma)\big)\Big)\Bigg|_{\mu=\mu_{\text{MAP}}}
= \frac{N}{\sigma^2}\Big(\bar{x} - \mu_{\text{MAP}}\Big)
- \frac{1}{\tau_0^2}\Big(\mu_{\text{MAP}} - \mu_0\Big)
= 0.
$$
...
\frac{N}{\sigma^2}\Big(\bar{x} - \mu_{\text{MAP}}\Big)
- \frac{1}{\tau_0^2}\Big(\mu_{\text{MAP}} - \mu_0\Big)
= \frac{N}{\sigma^2}\,\bar{x} + \frac{1}{\tau_0^2}\,\mu_0 
- \Bigg(\frac{N}{\sigma^2} + \frac{1}{\tau_0^2}\Bigg)\mu_{\text{MAP}} = 0.
$$

$$
\frac{N}{\sigma^2}\,\bar{x} + \frac{1}{\tau_0^2}\,\mu_0 
= \Bigg(\frac{N}{\sigma^2} + \frac{1}{\tau_0^2}\Bigg)\mu_{\text{MAP}}.
$$

And finally the posterior mode is:

$$
\mu_{\text{MAP}} = \frac{\frac{N}{\sigma^2}\,\bar{x} + \frac{1}{\tau_0^2}\,\mu_0}{\frac{N}{\sigma^2} + \frac{1}{\tau_0^2}}.
$$
...
Answer:
The posterior mode is similar to the posterior mean discussed in the lecture. So we used a conjugate normal prior on the mean, and the likelihood was also a normal distribution. The posterior is also a normal distribution. The posterior mean is the same as the posterior mode in this case because the posterior is symmetric. The posterior mean is the same as the posterior mode for Gaussian distribution.   
...
$$
p(\sigma \mid \mathbf{x}, \mu) \propto \prod_{n=1}^{N} \left( \frac{1}{\sqrt{2\pi \sigma^2}} \exp\left( -\frac{1}{2\sigma^2}(x_n - \mu)^2 \right) \right)
\times \lambda \exp(-\lambda \sigma)
$$

$$
\log\big(p(\sigma \mid \mathbf{x}, \mu)\big) \propto \sum_{n=1}^{N} \left( -\frac{1}{2}\log(2\pi) - \frac{1}{2}\log(\sigma^2) - \frac{1}{2\sigma^2}(x_n - \mu)^2 \right) + \log(\lambda) - \lambda \sigma
$$

$$
\log\big(p(\sigma \mid \mathbf{x}, \mu)\big) \propto \sum_{n=1}^{N} \left( -\frac{1}{2}\log(2\pi) \right)
+ \sum_{n=1}^{N} \left( -\frac{1}{2}\log(\sigma^2) \right)
- \frac{1}{2\sigma^2}\sum_{n=1}^{N}(x_n - \mu)^2 + \log(\lambda) - \lambda \sigma
$$

$$
\log\big(p(\sigma \mid \mathbf{x}, \mu)\big) \propto -\frac{N}{2}\log(\sigma^2)
...
Let's first multiply both sides by $N$ to simplify the expression:
$$
N v = \sum_{n=1}^{N} (x_n - \mu)^2
$$

Now, we can substitue in the log posterior on $\sigma$:
$$
\log\big(p(\sigma \mid \mathbf{x}, \mu)\big) \propto -\frac{N}{2}\log(\sigma^2) - \frac{N v}{2\sigma^2} - \lambda \sigma
$$
...
{
  -N * log(sigma) - 0.5*(N*v)/sigma^2 - lambda * sigma # your code here
}
```
...
What do you think?  
This does not look like a parabola. After the mode, the curve flattens.
...
$$
\sigma = g^{-1}(\phi) = \exp(\phi)
$$

The derivative is therefore simply equal to the function itself:
$$
\frac{d}{d\phi}\exp(\phi) = \exp(\phi)
$$


$$
\log\big(\exp(\phi)\big) = \phi
$$

The final derivative is : \(\phi\):
$$
\log\left|\frac{d}{d\phi}\Big(g^{-1}(\phi)\Big)\right| = \phi.
$$
...
What do you think?  
This one looks more parabolic compared to the unnormalized log posterior.
