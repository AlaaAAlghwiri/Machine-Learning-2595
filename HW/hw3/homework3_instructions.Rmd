---
title: "Intro to ML Spring 2025 Homework: 3"
...
The `log_bernoulli_pmf()` function is started for you in the code chunk below. It consists of two input arguments, `xobs`, and `prob`. The `xobs` argument is a numeric vector of observations of a binary variable encoded as 0 and 1. The `prob` argument is the event probability. You do not need to use a for-loop for this function, `R` will appropriately resize `prob` if the given `xobs` argument is a vector. As an example of what I mean:

```{r,eval=FALSE}
demo_vec <- function(xs,scalar) {
  xs * scalar + 3
}

4 |> demo_vec(scalar=4) # returns a scalar

seq(1,10) |> demo_vec(scalar=4) # returns a list

tibble(x = seq(1,50)) |> demo_vec(scalar=4) # returns a tibble
...
```{r, define_logbernoulli, eval=FALSE}
...
```{r, make_results_tibble_xa, eval=FALSE}
