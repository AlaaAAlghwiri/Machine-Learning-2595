...
$$ 
\log p(\mathbf{x} \mid \mu) = \sum_{n=1}^{N} \log \text{Bernoulli}(x_n \mid \mu)
$$
$$ 
\log p(\mathbf{x} \mid \mu) = \sum_{n=1}^{N} \left( x_n \log(\mu) + (1 - x_n) \log(1 - \mu) \right)
$$
$$
\log p(\mathbf{x} \mid \mu) = \log(\mu) \sum_{n=1}^{N} x_n + \log(1 - \mu) \sum_{n=1}^{N} (1 - x_n)
$$
...
$$ 
\log p(m \mid N, \mu) \propto m \log(\mu) + (N - m) \log(1 - \mu)
$$
...
$$ 
\log p(m \mid N, \mu) = \log \binom{N}{m} + \log \mu^m + \log (1 - \mu)^{N - m}
$$
$$ 
\log p(m \mid N, \mu) \propto m \log(\mu) + (N - m) \log(1 - \mu)
$$
...
b_new_inform_season
```
will put them all togther in one table
```{r}
season_output <- tibble::tibble(prior = c("uniform", "inform"),
                                a_new = c(a_new_uniform_season, a_new_inform_season),
                                b_new = c(b_new_uniform_season, b_new_inform_season))
print(season_output)
```
