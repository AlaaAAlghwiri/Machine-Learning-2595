...
subtitle: "Assigned February 6, 2025; Due: February 17, 2025"
...
MLE
```
MLE based on the class derivation ended up equal the number of events divided by the number of trials. And this equals the average number of hits (ones in this example) compared to the total number of trials (bats as in this example).
...
What do you think? 
Typically, the MLE equals the average or the expected value of the event.
...
$$ 
\log p \left(x_n \mid \mu \right) = \log \left( \mu ^ \left (x_n \right) * \left(1 - \mu \right)^\left(1-x_n \right) \right)
$$
$$ 
\log p \left(x_n \mid \mu \right) = \log \left( \mu ^ \left (x_n \right) \right) + \log \left( \left(1 - \mu \right)^\left ( 1 - x_n \right )\right)
$$
$$ 
\log p \left(x_n \mid \mu \right) = x_n * \log \left( \mu  \right) + \left (1 - x_n \right ) * \log \left( \left(1 - \mu \right)\right)
$$
...
{
        result <- (log(prob) * xobs) + (log(1 - prob) * (1 - xobs))
        return(result)
}
```
...
Insert code chunks.  
```{r}
log_bernoulli_pmf(x[1], prob = 0.25)
```

```{r}
log_bernoulli_pmf(x[2], prob = 0.25)
```
```{r}
log_bernoulli_pmf(x[3], prob = 0.25)
```
```{r}
log_bernoulli_pmf(x[4], prob = 0.25)
```
```{r}
log_bernoulli_pmf(x[5], prob = 0.25)
```
...
```{r}
log_bernoulli_pmf(x[1:5], prob = 0.25)
```

**What is the length of the returned result?**
The length of the returned result is **5** similar to the length of the input observation vector.

**How do the values compare to the previous question where you called the function separately for each observation?**  
the log likelihood values are the same as the single inputs shown in the previous problem.
...
$$ 
 log [p \left( \mathbf{x} \mid \mu \right)]= log \left( \prod_{n=1}^{N}\left(\mu^\left (x_n \right) * \left( 1 - \mu \right)^ \left(1- x_n  \right)\right) \right)
$$
...
```{r}
sum(log_bernoulli_pmf(xobs = x, prob = 0.25))
```
...
for (n in seq_along(mu_grid)){
        ...
        log_lik_xa[n] <- (log(mu_grid[n]) * sum(x)) + (log(1 - mu_grid[n]) * sum(1-x))
}
```
...
xa_results %>% 
        ggplot(aes(x = mu,
                   y = log_lik)) +
        geom_line() +
        geom_vline(xintercept = 0.4117647)
```  
...
**It's clear from the plot that the mle value maximizes the log likelihood such that it represents the maximum likelihood value. the curve increasing till reaching the mle value and start decreasing right after**
...
$$ 
\log \left( p \left( H \mid AB, \mu \right) \right) \propto log\left(\mu^\left(H\right)* (1 - \mu)^\left(AB- H\right)\right)
$$
...
The shape looks very much the same.

#### SOLUTION

What do you think?  
This figure is the same as the one in 2f. As mentioned in the slides, the binomial distribution is a sequence of independent Bernoulli trials. It might be a bit off because of not including the constant coefficient.
...
Both shapes are exactly the same, but they differ in their y-values.

#### SOLUTION

What do you think?  
The log-likelihood of binomial distribution is the same of the sequence of independent Bernoulli trials.
...
```{r}
number_of_trials_set1 <- a_uniform + b_uniform
number_of_trials_set1
number_of_trials_set2 <- a_inform + b_inform
number_of_trials_set2
```
...
Insert code chunks to answer the question. 
```{r}
expected_value_set1 <- a_uniform/(a_uniform+ b_uniform) 
expected_value_set1
expected_value_set2 <- a_inform/(a_inform + b_inform) 
expected_value_set2
```
...
**How do the two priors compare?**
Both priors are totally different. One of them is just a uniform distributin and no matter what the mu value, the likelihood is constant. On the other hand, the other distribution is approximately a bell-liked shape.

**Are there event probability values that are "ruled out" by either of the priors?**  
Yes, the uniform distribution probability is all ruled out.

#### SOLUTION

What do you think?  
I think that the distribution that was generated using the inform parameters is more informative and representative for the different mu values.
...
$$ 
\log \left( p \left( m \mid N, \mu \right) \right) \propto  log\left(\mu^\left(m\right)* (1 - \mu)^\left(N- m\right)\right)
$$
...
$$ 
 p \left( \mu \mid m, N \right) \propto Binomial(m \mid N,\mu) * Beta(\mu \mid a, b)
$$

$$ 
 p \left( \mu \mid m, N \right) \propto \mu^\left ( m\right) * (1- \mu)^\left( N - m \right)* \mu^\left ( a -1\right) * (1- \mu)^\left( b-1 \right)
$$
$$ 
 p \left( \mu \mid m, N \right) =  Beta(\mu \mid a + m, b + (N-m))
$$

$$ 
 p \left( \mu \mid m, N \right) =  Beta(\mu \mid a_{new}, b_{new})
$$
...
$$ 
log(Beta(\mu \mid a_{new}, b_{new})) = log(\mu^\left ( a_{new} -1\right) * (1- \mu)^\left( b_{new}-1 \right))
$$
$$ 
log(Beta(\mu \mid a_{new}, b_{new})) = (a_{new} -1) * log (\mu) + \left( b_{new}-1 \right) log(1- \mu)
$$

$$ 
\frac{\partial}{\partial \mu} log(Beta(\mu \mid a_{new}, b_{new})) = [(a_{new} -1)/\mu] + [(1-b_{new})/(1 - \mu)]
$$
...
$$ 
\frac{\partial}{\partial \mu} log(Beta(\mu \mid a_{new}, b_{new})) = 0
$$
$$ 
[(a_{new} -1)/\mu] + [(1-b_{new})/(1 - \mu)] = 0
$$


$$
\mu_{MAP}  = (a_{new} - 1)/(a_n{new} + b_{new} -2)
$$
...
Add as many code chunks as you feel are necessary. 
```{r Uniform prior}
a_new_uniform <- player_hits + a_uniform
a_new_uniform
...
```{r Inform prior}
a_new_inform <- player_hits + a_inform
a_new_inform
...
```{r}
posterior_outputs <- tibble::tibble(
...
posterior_outputs %>% 
  mutate(post_mean = a_new /(a_new + b_new),
         post_mode = (a_new - 1) / (a_new + b_new - 2),
         post_q05 = qbeta(0.05, a_new, b_new),
         post_q95 = qbeta(0.95, a_new, b_new)) %>% 
  select(prior_type, starts_with('post_'))
```


##### Inform prior
```{r, Inform prior summary stats}
mean_inform <- a_new_inform/(a_new_inform + b_new_inform)
paste0("the mean of the uniform prior = ", mean_inform)

mode_inform <- (a_new_inform - 1)/(a_new_inform + b_new_inform - 2)
paste0("the mode of the uniform prior = ", (mode_inform))

fifth_percentile_inform <- qbeta(0.05, 
                                  shape1 = a_new_inform, 
                                  shape2 = b_new_inform) 
paste0("the fifth percentile of the inform prior = ", (fifth_percentile_inform))

nintyfivethe_percentile_inform <- qbeta(0.95, 
                                         shape1 = a_new_inform, 
                                         shape2 = b_new_inform) 
paste0("the 95th percentile of the inform prior = ", (nintyfivethe_percentile_inform))
...
tibble::tibble(
  ...
  mutate(beta_pdf = dbeta(x = mu,
                          shape1 = a_new_uniform,
                          shape2 = b_new_uniform)) %>%
        ggplot(aes(x = mu,
               y = beta_pdf)) +
        geom_line()
```
...
tibble::tibble(
  ...
  mutate(beta_pdf_inform = dbeta(x = mu,
                          shape1 = a_new_inform,
                          shape2 = b_new_inform),
         beta_pdf_uniform = dbeta(x = mu,
                          shape1 = a_new_uniform,
                          shape2 = b_new_uniform)) %>%
        ggplot(aes(x = mu)) +
        geom_line(aes(y = beta_pdf_uniform)) + 
        geom_line(aes(y = beta_pdf_inform), linetype = 'dashed')
```

**Based on your results, how would you describe the differences in the posterior belief based on the two sets of priors?**  
For the posterior with uniform prior, the 0.95 quantile is 0.41 and the 0.05 quantile is 0.2. On the other hand, the posterior with the inform prior has a 0.95 quantile equals to 0.35 and the 0.05 quantile equals to 0.2. 

#### SOLUTION

What do you think?  
The posterior with uniform prior shows that mu spans from 0.2 to 0.55 (area under the curve). On the other hand, the posterior with an inform prior shows that the mu values can't be approximately more than 0.45. 
...
Add as many code chunks as you feel are necessary.  
```{r Uniform prior _}
a_new_uniform_season <- season_hits + a_uniform
a_new_uniform_season
b_new_uniform_season <- b_uniform + (season_atbats - season_hits)
b_new_uniform_season
```

```{r Inform prior _}
a_new_inform_season <- season_hits + a_inform
a_new_inform_season
b_new_inform_season <- b_inform + (season_atbats - season_hits)
b_new_inform_season
```
...
##### Uniform prior
```{r, uniform prior summary stats _}
mean_uniform <- a_new_uniform/(a_new_uniform + b_new_uniform)
paste0("the mean of the uniform prior = ", mean_uniform)

mode_uniform <- (a_new_uniform - 1)/(a_new_uniform + b_new_uniform - 2)
paste0("the mode of the uniform prior = ", (mode_uniform))

fifth_percentile_uniform <- qbeta(0.05, 
                                  shape1 = a_new_uniform, 
                                  shape2 = b_new_uniform) 
fifth_percentile_uniform

nintyfivethe_percentile_uniform <- qbeta(0.95, 
                                         shape1 = a_new_uniform, 
                                         shape2 = b_new_uniform) 
nintyfivethe_percentile_uniform
```

##### Inform prior
```{r, Inform prior summary stats _}
mean_inform_season <- a_new_inform_season/(a_new_inform_season + b_new_inform_season)
paste0("the mean of the uniform prior = ", mean_inform_season)

mode_inform_season <- (a_new_inform_season - 1)/(a_new_inform_season + b_new_inform_season - 2)
paste0("the mode of the uniform prior = ", (mode_inform_season))

fifth_percentile_inform_season <- qbeta(0.05, 
                                  shape1 = a_new_inform_season, 
                                  shape2 = b_new_inform_season) 
paste0("the fifth percentile of the inform prior = ", (fifth_percentile_inform_season))

nintyfivethe_percentile_inform_season <- qbeta(0.95, 
                                         shape1 = a_new_inform_season, 
                                         shape2 = b_new_inform_season) 
paste0("the 95th percentile of the inform prior = ", (nintyfivethe_percentile_inform_season))
...
```{r, solutioN_07c_a_, eval=TRUE}
tibble::tibble(
  mu = mu_grid
) %>% 
  mutate(beta_pdf = dbeta(x = mu,
                          shape1 = a_new_uniform_season,
                          shape2 = b_new_uniform_season)) %>%
        ggplot(aes(x = mu,
               y = beta_pdf)) +
        geom_line()
```
...
tibble::tibble(
  ...
  mutate(beta_pdf_inform_season = dbeta(x = mu,
                          shape1 = a_new_inform_season,
                          shape2 = b_new_inform_season),
         beta_pdf_uniform_season = dbeta(x = mu,
                          shape1 = a_new_uniform_season,
                          shape2 = b_new_uniform_season)) %>%
        ggplot(aes(x = mu)) +
        geom_line(aes(y = beta_pdf_uniform_season)) + 
        geom_line(aes(y = beta_pdf_inform_season), linetype = 'dashed')
```

**Describe the influence of the prior on the posterior when the sample size is small vs large.**  
The prior impacted the posterior significantly in the small sample size shown in previous questions. When the sample size is large, the impact of the prior is very small as we can see from the figure above. both different priors ended up with the approximately the same posterior for large sample size.

#### SOLUTION

What do you think?  
I think that the power of Bayesian approach shines for small sample size but doesn't matter for large sample size.
