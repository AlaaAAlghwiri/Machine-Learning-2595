---
title: "INFSCI 2595 Spring 2025 Homework: 6"
subtitle: "Assigned April 3, 2024; Due: April 14, 2024"
...
```{r, read_data_1}
df1 <- readr::read_csv('hw6_binary.csv', col_names = TRUE)
```
...
What do you think?  
The binary outcome is not balanced. The event class is somehow the minority class.
...
What do you think?  
Obviously, none of those fits seem to work well for the data. 
...
What do you think?  
The smooth function tries to fit a quadratic function to the data, but it does not seem to work well. The data is not quadratic in nature and just the z representation can fit such a kind of outcomes.
...
You must name your models `modA` through `modE`.  
...
```{r}
modA <- glm( y ~ x1, data = df1)

modB <- glm( y ~ x2 + x3, data = df1)

modC <- glm( y ~ x1 + x2 + x3, data = df1 )

modD <- glm( y ~ x1 * (x2 + x3), data = df1 )

modE <- glm( y ~ x1 + x2 * x3, data = df1 )
...
**Which of the 5 models is the best? Which of the 5 models is the second best?**  
...
glm_mle_results %>% 
  ...
  theme_bw()
```
Based on the AIC and BIC values, the best model is model C and the second best is model E. The AIC and BIC values are used to select the best model. The lower the AIC and BIC values, the better the model.
...
modC %>% 
  ...
  theme_bw()
```
x2 and x1b are significant in the model C. 
...
modE %>% 
  ...
  theme_bw()
```
x2 and x1b are significant in the model E.
...
You will fit 5 Bayesian logistic regression models using the same linear predictor trend expressions that you used in the non-Bayesian logistic regression models. You will program the log-posterior function in the same style as the linear model log-posterior functions. This allows you to use the same Laplace Approximation strategy to execute the Bayesian inference.  
...
```{r}
Xmat_A <- model.matrix(modA$formula, data = df1)
Xmat_B <- model.matrix(modB$formula, data = df1)
Xmat_C <- model.matrix(modC$formula, data = df1)
Xmat_D <- model.matrix(modD$formula, data = df1)
Xmat_E <- model.matrix(modE$formula, data = df1)
...
You will use `my_laplace()` to execute the Laplace Approximation for all 5 models. You must use an initial guess of zero for all unknowns for each model.  

**Perform the Laplace Approximation for all 5 models. Assign the results to the `laplace_A` through `laplace_H` objects. The names should be consistent with the design matrices and lists of required information. Thus, `laplace_A` must correspond to the `info_A` and `Xmat_A` objects.**  
...
tibble::tibble(
  ...
  post_sd = sqrt( diag( laplace_A$var_matrix ) )
) %>% 
  mutate(ci_lwr = post_mean - 2 * post_sd,
         ci_upr = post_mean + 2 * post_sd) %>% 
  select(term, ci_lwr, ci_upr)
```
```{r}
modA %>% summary()
```
...
You trained 5 Bayesian logistic regression models. Let's identify the best using the Evidence based approach!  

**Calculate the posterior model weight associated with each of the 5 models. Create a bar chart that shows the posterior model weight for each model. The models should be named `'A'` through `'H'`.**  
...
#### SOLUTION
This approach identifies model B as the best model and this is different from what i got earlier.
...
You will not make predictions from all 5 models that you previously trained. Instead, you will focus on model B, model D, and model E.  

**You must define the vizualization grid design matrices consistent with the model B, model D, and model E formulas. You must name the design matrices `Xviz_D`, `Xviz_G`, and `Xviz_H`. You must create the design matrices using the `viz_grid` dataframe which was defined at the start of Problem 04.**  

#### SOLUTION

```{r}
Xviz_B <- model.matrix(~ x2 + x3, data = viz_grid)
Xviz_E <- model.matrix( ~ x1 + x2 * x3, data = viz_grid )
...
post_pred_summary_E <- summarize_logistic_pred_from_laplace(laplace_E, Xviz_E, 2500)
...
```
```{r}
viz_bayes_logpost_preds(post_pred_summary_E, viz_grid)
...
An alternative approach to training many models of varying complexity is to train a single complex model and use regularization to "turn off" the unimportant features. This way we can find out if the most complex model can be turned into a simpler model of just the most key features we need!  
...
**Train, assess, and tune the `glmnet` elastic net model consistent with model E with the defined resampling scheme. Assign the result to the `enet_default` object and display the result to the screen.**  
...
enet_tune$bestTune
```
Alpha value of 0.3 and lambda of 0.01 are best parameters.
...
Some of the parameters like x1C are now zero. 
...
varImp(enet_tune)
```
(x3^2) is the most important feature in this case.
...
nnet_default <- train( outcome ~ x1 + x2 + x3,
                       data = df_caret,
                       method = 'nnet',
                       metric = my_metric,
                       preProcess = c("center", "scale"),
...
rf_default <- train( outcome ~ x1 + x2 + x3,
                       data = df_caret,
                       method = 'rf',
                       metric = my_metric,
                       #preProcess = c("center", "scale"),
...
X3 is the most important feature and this agrees with what i had previously for the elastic net model. The difference is that in the elastic net model, we've included interaction terms but in the random forest model we have not.
...
#### SOLUTION
The elastic net model is the best model in this case but with a very small amount. The accuracy and confidence interval for all models overlap with each other. In this case and for interpretability and simplicity reasons, I would go with the elastic net model. 
