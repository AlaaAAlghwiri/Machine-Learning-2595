...
```{r, plot_weight}
abalone |>
        ggplot(aes(x = WholeWeight)) +
        geom_histogram(bins = 15, color = 'black', 
                       fill = 'antiquewhite4', size = 1.55,
                       alpha = 0.5) +
        labs(title = 'Histogram of WholeWeight')
```
...
```{r, solution_1g}
abalone |>
        ggplot(aes(x = WholeWeight, fill = Type)) +
        geom_histogram() +
        labs(title = 'Histogram of WholeWeight by Type')
```
...
abalone |>
        ...
        facet_wrap(~Type) +
        labs(title = 'Histogram of WholeWeight by Type (Faceted)')
```
...
Based on the median values shown by each color represented by a boxplot, diamonds with colors D and E are the cheapest. On the other hand, diamonds with colors I and J are the most expensive. We can clearnly see lot's of seemingly extreme values in the upper side of the boxplot
...
```{r, solution_02e, fig.width= 14, fig.height=10}
diamonds |>
        ggplot(aes(x= color, y = price)) +
...
In part 2b, we were able to identify basic summaries based only on how the price is changing with the different color categories. On the other hand, in 2f,we were able to incorporate information from other variables as well. In one 2-D graph, we were able to incorporate all 4'C's variables. After filtering for `cut == "Ideal"` and `clarity == "IF"`, we can tell that diamonds prices for carat greater than the median vallue are higher than the prices for carat less than the median value. Also,diamonds that their carat above the median value and their color is D or E have the highest prices. color D for both types of carat is the most expensive as well.

we are now able to see how the price is changing with the color variables taking into account the carat variable. And how the carat variable is changing with the color variable. This is a more detailed analysis of the data. Now we can say for the category of ideal cut and IF clarity, the price of those diamonds that their carat is greater than the median carat is higher than the price of those diamonds that their carat is less than the median carat. This obviously completely contradict with my conclusion in 2b when is said that color D and E are the cheapest. This obviously shows the importance of incorporating more variables in one visualization to uncover hidden patterns and ggplot2 is great in this regard.
...
From the coefficient plot, we can see that the confidence interval for the `Height` coefficient does not contain 0, thus we say that it is statistically significant.
...
```{r, solution_04b_3}
coefplot::coefplot(mod1, coefficients = c('Height'), 
...
Based on the coefficient plot, not all the variables are significant. The `Height` variable is not significant as the zero value is part of its confidence interval. On the other hand, `WholeWeight` and the intercept are significant. The plot displays 3 coefficients including the intercept.
...
$$
\left( \mathbf{X_1}^\mathbf{Z^{Y}} \right)
$$
