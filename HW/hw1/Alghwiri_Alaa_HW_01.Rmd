...
```{r, solution_02b, fig.width= 14, fig.height=8}
diamonds |>
...
```{r, solution_02c, fig.width= 14, fig.height=8}
diamonds |>
        ggplot(aes(x= color, y = price)) +
        geom_boxplot() +
        facet_wrap(~cut)
...
```{r, solution_02d, fig.width= 14, fig.height=10}
diamonds |>
        ggplot(aes(x= color, y = price)) +
        geom_boxplot() +
...
```{r, solution_02e, fig.width= 16, fig.height=10}
diamonds |>
        ggplot(aes(x= color, y = price)) +
...
In part 2b, we were able to identify basic summaries based only on how the price is changing with the different color categories. On the other hand, in 2f,we were able to incorporate information from other variables as well. In one 2-D graph, we were able to incorporate all 4 C's variables. After filtering for `cut == "Ideal"` and `clarity == "IF"`, we can tell that diamonds prices for carat greater than the median value are higher than the prices for carat less than the median value. Also,diamonds with carat above the median value and their color is D or E have the highest prices. color D for both types of carat is the most expensive as well. This is obviously and completely contradicting my conclusion in 2b when said that color D and E are the cheapest ones!! This obviously shows the importance of incorporating more variables in one visualization to uncover hidden patterns and ggplot2 is great in this regard.
