...
Based **only** on the training set performance metrics, polynomial order 9 seems to be the best model. The reason it seems so, is that it has the lowest RMSE value and the highest R^2 value tie with polynomial order 8. Both performance metrics agree that polynomial 9 is the best here. Apparently, polynomial order 1 is out of this candidates list as I have seen a drastic decrease in RMSE and increase in R^2 values once I tried the polynomial order 2 and the rest. 
...
your response here.  
- 6 observations are associated with false positive.
- 53 observations are associate with true negative.
...
```{r, solution_05d}
model_class_0.5 %>%
        summarise(Sensitivity = (sum(pred_class == "event" & obs_class == "event"))/(sum(pred_class == "event" & obs_class == "event") + sum(pred_class == "event" & obs_class == "non_event")),
                  FPR = 1 - (sum(pred_class == "non_event" & obs_class == "non_event"))/(sum(pred_class == "non_event" & obs_class == "non_event") + sum(pred_class == "non_event" & obs_class == "event")))
```
...
model_class_0.7 <- model_class_0.7 <- model_pred_df %>%
...
```{r, solution_06b}
model_class_0.7 %>%
        summarise(Sensitivity = sum(obs_class == "event" & pred_class == "event")/(sum(obs_class == "event" & pred_class == "event") + sum(obs_class == "non_event" & pred_class == "event")),
                  Specificity = sum(obs_class == "non_event" & pred_class == "non_event")/(sum(obs_class == "non_event" & pred_class == "non_event") + sum(obs_class == "event" & pred_class == "non_event")))
```
...
```{r, solution_06e}
tibble(model_threshold = c(0.3, 0.5, 0.7),
       Sensitivity = c(0.73, 0.85, 0.96),
       `FPR = 1 - Specificity` = c(0.31, 0.37, 0.41)) %>%
        ggplot(aes(x = `FPR = 1 - Specificity`,
                   y = Sensitivity)) + 
        geom_point() +
        geom_text(aes(label = model_threshold),
                  nudge_y = 0.03) +
        geom_abline(slope = 1, 
                    intercept = 0) +
