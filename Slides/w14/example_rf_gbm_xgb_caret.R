...
###
### fitting without resampling is to just demonstrate the concepts of
### the random forest
...
### we will use cross-validation to decide the value of mtry
### that we should use!!! This way the OOB error is NOT used
### to identify the best model. We use resampling to provide
### extra assurance about the model's overall performance!!!

### specify cross-validation to tune mtry
### this example uses 5-fold CV 
ctrl_k05 <- trainControl(method = "cv", number = 5)

### use cross-validation to tune mtry
### a custom mtry grid is specified directly within tuneGrid
set.seed(12341)
...
### the $results field is accessed directly to make a custom plot
...
### a smaller learning rate - gbm calls learning rate the shrinkage
...
### 
### nthread is set to 1 to make sure the CPU is not overloaded
### 
set.seed(12341)
