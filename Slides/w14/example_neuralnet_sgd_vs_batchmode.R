...
### this example does not setup the most computationally efficient SGD algorithm
...
### calculate the gradient, but do so numerically to limit the
### amount of code required for the example
...
opt_iterations_0 %>% 
  ...
  geom_hline(yintercept = 0, color = 'grey', linetype = 'dotted', size = 1.2) +
  geom_hline(data = tibble::tibble(optim_est = two_hidden_fit_a$par) %>% 
               tibble::rowid_to_column("theta_id"),
             mapping = aes(yintercept = optim_est),
             color = "red", linetype = "dashed", size = 1.) +
  geom_line(mapping = aes(group = theta_id), size = 1.1) +
...
### instead of looking at the number of iterations, instead look at the
### number of EPOCHS
opt_iterations_0 %>% tibble::rowid_to_column() %>% 
  mutate(type = 'BATCH') %>% 
  bind_rows(sgd_iterations_0 %>% tibble::rowid_to_column() %>% 
  ...
  bind_rows(sgd_batch_12 %>% tibble::rowid_to_column() %>% 
              mutate(type = "SGD-12")) %>% 
  tidyr::separate(type, c("alg_type", "batch_size"),
                  sep = "-",
                  fill = 'right',
                  remove = FALSE) %>% 
  mutate(batch_size = as.numeric(batch_size)) %>% 
  mutate(batch_size = ifelse(is.na(batch_size),
                             nrow(two_hidden_sse_info$design_matrix),
                             batch_size)) %>% 
  mutate(iter_per_epoch = nrow(two_hidden_sse_info$design_matrix) / batch_size) %>% 
  mutate(epoch = iter_id / iter_per_epoch) %>% 
  ggplot(mapping = aes(x = epoch, y = theta)) +
  geom_hline(yintercept = 0, color = 'grey', linetype = 'dotted', size = 1.2) +
  geom_hline(data = tibble::tibble(optim_est = two_hidden_fit_a$par) %>% 
               tibble::rowid_to_column("theta_id"),
             mapping = aes(yintercept = optim_est),
