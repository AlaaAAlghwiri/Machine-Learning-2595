...
### try several other initial guess values...the gradient value changes!!!
numDeriv::grad(negative_logpost, c(254, 3), logpost_info = info_use)

numDeriv::grad(negative_logpost, c(254, 0), logpost_info = info_use)

numDeriv::grad(negative_logpost, c(254, -0.5), logpost_info = info_use)
...
### other parameter values are associated with different second derivatives!!!
numDeriv::hessian(negative_logpost, c(254, 3), 
                  method="Richardson", method.args=list(), 
                  info_use)

numDeriv::hessian(negative_logpost, c(254, 0), 
                  method="Richardson", method.args=list(), 
                  info_use)

numDeriv::hessian(negative_logpost, c(254, -0.5), 
                  method="Richardson", method.args=list(), 
                  info_use)
...
### how does the Newton step work??? let's try with an example guess
H_try <- numDeriv::hessian(negative_logpost, c(250, -1), 
                           method="Richardson", method.args=list(), 
                           info_use)

H_try

g_try <- numDeriv::grad(negative_logpost, c(250, -1), 
                        logpost_info = info_use)

g_try

### Newton update is the negative inverse of the Hessian multiplied 
### by the gradient!
-solve(H_try) %*% as.matrix(g_try)
...
### number of iterations per initial guess?
purrr::map2_dfr(grad_desc_opt_results_all, 
                seq_along(grad_desc_opt_results_all),
                function(ll, id){ll %>% mutate(guess_id = id)}) %>% 
  mutate(opt_type = "Gradient Descent") %>% 
  filter(!is.na(lik_mu)) %>% 
  group_by(opt_type, guess_id) %>% 
  summarise(num_iter = max(iter_id),
            .groups = 'drop') %>% 
  ggplot(mapping = aes(x = guess_id)) +
  geom_linerange(mapping = aes(ymin = 0,
                               ymax = num_iter)) +
  geom_point(mapping = aes(y = num_iter), size = 3) +
  facet_wrap(~opt_type) + 
  scale_x_continuous(breaks = seq_along(grad_desc_opt_results_all)) +
  theme_bw() +
...
### number of iterations for each initial guess when we use the
...
purrr::map2_dfr(grad_desc_opt_results_small_step, 
  ...
  geom_point(mapping = aes(y = num_iter), size = 3) +
  facet_wrap(~opt_type) + 
  scale_x_continuous(breaks = seq_along(grad_desc_opt_results_small_step)) +
  theme_bw() +
  theme(panel.grid.minor.x = element_blank())

### what if we tried a very large learning rate?
### try 1...CRASHES!!!
### try 0.5...CRASHES!!!
### usinig 0.05 does NOT crash!!!
grad_desc_opt_results_large_step <- purrr::map(init_guess_list,
                                               run_tidy_optimize,
                                               num_steps = 1000,
                                               grad_func = make_grad_desc,
                                               logpost_info = info_use,
...
purrr::map2_dfr(grad_desc_opt_results_large_step, 
                seq_along(grad_desc_opt_results_large_step),
                function(ll, id){ll %>% mutate(guess_id = id)}) %>% 
  mutate(opt_type = "Gradient Descent large learning rate") %>% 
  filter(!is.na(lik_mu)) %>% 
  ggplot(mapping = aes(x = iter_id, y = grad_l2_norm)) +
  geom_line(mapping = aes(group = interaction(guess_id),
                          color = as.factor(guess_id)),
...
### l2 norm of the gradient vs iteration
purrr::map2_dfr(grad_desc_opt_results_large_step, 
                seq_along(grad_desc_opt_results_large_step),
                function(ll, id){ll %>% mutate(guess_id = id)}) %>% 
  mutate(opt_type = "Gradient Descent large learning rate") %>% 
...
### can we find a learning where the result looks really weird???
### focus on 1 initial guess
grad_desc_opt_results_weird_step <- purrr::map(init_guess_list[7],
                                               run_tidy_optimize,
                                               num_steps = 1000,
                                               grad_func = make_grad_desc,
                                               logpost_info = info_use,
...
purrr::map2_dfr(grad_desc_opt_results_weird_step, 
                seq_along(grad_desc_opt_results_weird_step)+6,
                function(ll, id){ll %>% mutate(guess_id = id)}) %>% 
  mutate(opt_type = "Gradient Descent small learning rate") %>% 
  filter(!is.na(lik_mu)) %>% 
  ggplot(mapping = aes(x = iter_id, y = grad_l2_norm)) +
...
viz_compare_opt_results <- function(graddesc_df, newton_df, guess_id)
{
  ...
  gg <- log_post_refine_df %>% 
    ...
    geom_text(data = init_values %>% mutate(guess_id = guess_id),
              mapping = aes(label = guess_id),
              color = 'black', size = 8) +
    geom_path(data = graddesc_df %>% filter(!is.na(lik_mu)),
              mapping = aes(x = lik_mu, y = lik_phi,
                            color = 'Gradient Descent'),
              size= 1.0) +
    geom_path(data = newton_df %>% filter(!is.na(lik_mu)),
              mapping = aes(x = lik_mu, y = lik_phi,
                            color = "Newton Method"),
              size= 1.0) +
    geom_point(data = graddesc_df %>% filter(!is.na(lik_mu)),
               mapping = aes(x = lik_mu, y = lik_phi,
                             color = 'Gradient Descent')) +
    geom_point(data = newton_df %>% filter(!is.na(lik_mu)),
               mapping = aes(x = lik_mu, y = lik_phi,
                             color = 'Newton Method'),
               size = 2.21) +
    scale_fill_viridis_c(guide = 'none', 
...
### what about the behavior of that WEIRD learning rate with gradient
### descent associated with that one initial guess?
viz_compare_opt_results(grad_desc_opt_results_weird_step[[1]],
                        newton_opt_results_all[[7]],
                        7)
